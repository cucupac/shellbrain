...
def test_db_boot_should_always_resolve_the_runtime_configured_dsn_env(
    monkeypatch,
) -> None:
...
def test_admin_db_boot_should_always_resolve_the_runtime_configured_admin_dsn_env(
    monkeypatch,
) -> None:
...
def test_optional_admin_db_dsn_should_return_none_when_machine_config_is_corrupt(
    monkeypatch,
) -> None:
    """optional admin db boot should suppress unreadable machine config errors."""

    monkeypatch.setattr(
        "app.startup.admin_db.try_load_machine_config", lambda: (None, "corrupt toml")
    )
...
def test_db_boot_should_prefer_machine_config_over_legacy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.startup.db.try_load_machine_config", lambda: (machine_config, None)
    )
...
def test_db_boot_should_fail_cleanly_when_machine_config_is_corrupt(
    ...
    monkeypatch.setattr(
        "app.startup.db.try_load_machine_config", lambda: (None, "corrupt toml")
    )
...
def test_embedding_boot_should_use_local_only_when_machine_config_is_ready(
    monkeypatch,
) -> None:
...
def test_db_boot_should_support_external_machine_config(monkeypatch) -> None:
    ...
    assert (
        get_db_dsn()
        == "postgresql+psycopg://shellbrain_app:app_secret@db.example.com:5432/shellbrain"
    )
