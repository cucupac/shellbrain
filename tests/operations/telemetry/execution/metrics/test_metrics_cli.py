...
def test_metrics_command_should_generate_all_repo_artifacts_and_launch_browser_dashboard(
    ...
    monkeypatch.setattr(startup_cli, "warn_or_fail_on_unsafe_app_role", lambda: None)
    monkeypatch.setattr(
        "app.startup.db.get_optional_db_dsn", lambda: "postgresql://metrics-test"
    )
    ...
    repo_ids = {
        json.loads(path.read_text(encoding="utf-8"))["repo_id"] for path in json_paths
    }
    assert repo_ids == {"github.com/example/one", "github.com/example/two"}
    assert all(
        "Utility score trend" in path.read_text(encoding="utf-8") for path in md_paths
    )
...
def test_metrics_command_should_print_empty_message_when_no_repo_metrics_exist(
    monkeypatch, capsys
) -> None:
...
def test_metrics_command_should_reject_global_repo_target_options(
    monkeypatch, capsys
) -> None:
        ...
        "daily_series": [
            {
                ...
                "rolling_value": current - 0.1,
            },
            {
                "date": "2026-03-27",
                "value": current,
                "sample_count": sample_count - (sample_count // 2),
                "rolling_value": current - 0.05,
            },
        ],
    }
