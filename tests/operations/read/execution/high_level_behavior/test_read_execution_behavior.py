...
def test_read_enforces_repo_visibility_and_include_global_scope(
    ...
    request_without_global = make_read_request(
        repo_id="repo-a",
        query="deployment issue",
        include_global=False,
    )
    request_with_global = make_read_request(
        repo_id="repo-a",
        query="deployment issue",
        include_global=True,
    )
...
def test_read_enforces_hard_output_cap_equal_to_limit(
    ...
    request = make_read_request(repo_id="repo-a", query="deployment issue", limit=2)
    with uow_factory() as uow:
        result = execute_read_memory(request, uow)
...
def test_read_returns_empty_pack_when_no_candidates_pass(
    ...
    request = make_read_request(repo_id="repo-a", query="nothing in this database")
    with uow_factory() as uow:
        result = execute_read_memory(request, uow)

    assert item_ids(result) == []
