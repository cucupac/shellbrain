...
def test_install_host_assets_auto_should_install_the_default_codex_and_claude_set(monkeypatch, tmp_path: Path) -> None:
    ...
    assert any(line.startswith("Codex skill: installed at ") for line in result.lines)
    assert any(line.startswith("Claude skill: installed at ") for line in result.lines)
...
def test_install_host_assets_should_not_overwrite_unmanaged_codex_skill_without_force(monkeypatch, tmp_path: Path) -> None:
    ...
    assert result.lines == [
        f"Codex skill: skipped (unmanaged install exists at {unmanaged_root}; rerun with --force to replace)"
    ]
...
def test_install_host_assets_should_update_managed_codex_skill_idempotently(monkeypatch, tmp_path: Path) -> None:
    ...
    assert any(line.startswith("Codex skill: installed at ") for line in first.lines)
    assert second.lines == [f"Codex skill: updated at {skill_root}"]
