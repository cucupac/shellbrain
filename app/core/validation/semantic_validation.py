"""This module defines semantic validation checks for domain and link constraints."""
...
def validate_create_semantics(request: MemoryCreateRequest) -> list[ErrorDetail]:
    """This function validates semantic constraints for create operations."""
...
def validate_update_semantics(request: MemoryUpdateRequest) -> list[ErrorDetail]:
    """This function validates semantic constraints for update operations."""
    ...
    if isinstance(update, FactUpdateLinkUpdate):
        ...
        if update.old_fact_id == request.memory_id or update.new_fact_id == request.memory_id:
            errors.append(
                ErrorDetail(
                    code=ErrorCode.SEMANTIC_ERROR,
                    message="memory_id is the change memory id and cannot equal old_fact_id/new_fact_id",
                    field="memory_id",
                )
            )
    return errors
