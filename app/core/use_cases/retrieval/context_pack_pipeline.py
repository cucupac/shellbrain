...
def _hydrate_pack_items(
    pack: dict[str, Any], memories: IMemoriesRepo
) -> dict[str, Any]:
    ...
    if missing_memory_ids:
        hydrated_memories = {
            memory.id: memory for memory in memories.list_by_ids(missing_memory_ids)
        }
        for section_name in ("direct", "explicit_related", "implicit_related"):
            for item in pack[section_name]:
                ...
                if memory is None:
                    raise ValueError(
                        f"Missing hydrated memory for context-pack item: {memory_id}"
                    )
...
def _iso(value: datetime | None) -> str | None:
    """Return a stable ISO timestamp for JSON-facing read-pack fields."""
