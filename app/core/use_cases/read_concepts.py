...
from app.core.contracts.requests import MemoryReadRequest, ReadConceptsExpandRequest
...
def _auto_concept_items(
    *,
    ...
    for link_match in concepts.find_concepts_for_memory_ids(repo_id=request.repo_id, memory_ids=memory_ids):
    ...
    for concept_id, candidate in candidates.items():
        bundle = concepts.get_concept_bundle(repo_id=request.repo_id, concept_ref=concept_id)
        if bundle is None:
            continue
        score = float(candidate["score"]) * _freshness_multiplier(bundle)
        ranked.append((score, str(bundle["concept"].slug), {**candidate, "bundle": bundle}))
...
def _explicit_concept_items(
    *,
    ...
    for concept_ref in concept_expand.refs:
        bundle = concepts.get_concept_bundle(repo_id=request.repo_id, concept_ref=concept_ref)
        if bundle is None:
...
def _append_linked_memory_reason(reasons: list[dict[str, Any]], link_match: dict[str, Any]) -> None:
    role = str(link_match.get("role") or "")
    existing = next((item for item in reasons if item.get("reason") == "linked_memory" and item.get("role") == role), None)
    if existing is None:
...
def _append_query_reason(reasons: list[dict[str, Any]], query_match: dict[str, Any]) -> None:
    reason = str(query_match.get("reason") or "query_match")
    matched = str(query_match.get("matched") or "")
    if any(item.get("reason") == reason and item.get("matched") == matched for item in reasons):
        return
...
def _orientation(concept: Concept, claims: list[ConceptClaim]) -> str:
    definition = next(
        (claim.text for claim in _sort_claims(claims) if claim.claim_type.value == "definition" and claim.lifecycle.status == ConceptLifecycleStatus.ACTIVE),
        None,
    )
...
def _key_claims(claims: list[ConceptClaim]) -> list[dict[str, Any]]:
    active_claims = [claim for claim in _sort_claims(claims) if claim.lifecycle.status == ConceptLifecycleStatus.ACTIVE]
...
def _relation_payloads(bundle: dict[str, Any], concepts: IConceptsRepo) -> list[dict[str, Any]]:
    ...
    endpoints = {
        concept.id: concept
        for concept in concepts.list_concepts_by_ids(repo_id=bundle["concept"].repo_id, concept_ids=endpoint_ids)
    }
    payloads: list[dict[str, Any]] = []
    for relation in sorted(relations, key=lambda item: (item.predicate.value, item.subject_concept_id, item.object_concept_id)):
        payloads.append(
            {
                "id": relation.id,
                "predicate": relation.predicate.value,
                "subject": _concept_ref_payload(endpoints.get(relation.subject_concept_id), relation.subject_concept_id),
                "object": _concept_ref_payload(endpoints.get(relation.object_concept_id), relation.object_concept_id),
...
def _grounding_payloads(bundle: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    payloads: list[dict[str, Any]] = []
    for grounding in sorted(bundle["groundings"], key=lambda item: (item.role.value, item.anchor_id)):
...
def _memory_link_payloads(bundle: dict[str, Any], memories: IMemoriesRepo) -> list[dict[str, Any]]:
    links: list[ConceptMemoryLink] = list(bundle["memory_links"])
    memory_by_id: dict[str, Memory] = {memory.id: memory for memory in memories.list_by_ids([link.memory_id for link in links])}
    payloads: list[dict[str, Any]] = []
...
def _evidence_payloads(bundle: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        for evidence in sorted(bundle["evidence"], key=lambda item: (item.target_type.value, item.target_id, item.evidence_kind.value, item.id))
    ]
...
def _concept_ref_payload(concept: Concept | None, fallback_id: str) -> dict[str, Any]:
    ...
    return {"id": concept.id, "ref": concept.slug, "name": concept.name, "kind": concept.kind.value}
