...
def build_agent_behavior_report(
    *,
    ...
    checkpoint_event_rows: list[dict[str, object]],
) -> dict[str, Any]:
    ...
    checkpoint_rows = _parse_checkpoint_event_rows(checkpoint_event_rows)

    pre_report = _build_window_report(
        operation_rows=[
            row for row in operation_rows if _coerce_utc(row["created_at"]) < cutoff_at
        ],
        read_rows=[
            row for row in read_rows if _coerce_utc(row["created_at"]) < cutoff_at
        ],
        write_rows=[
            row for row in write_rows if _coerce_utc(row["created_at"]) < cutoff_at
        ],
        checkpoint_rows=[
            row for row in checkpoint_rows if _coerce_utc(row["created_at"]) < cutoff_at
        ],
    )
    post_report = _build_window_report(
        ...
        checkpoint_rows=[
            ...
            if _coerce_utc(row["created_at"]) >= cutoff_at
        ],
    )
    return {
        "window": {
...
def _parse_checkpoint_event_rows(
    rows: list[dict[str, object]],
) -> list[dict[str, object]]:
    """Return parsed SB checkpoints from transcript event rows."""
...
def _build_window_report(
    *,
    ...
    repo_host_pairs = sorted(
        {(str(row["repo_id"]), str(row["host_app"])) for row in operation_rows}
    )
    return {
        ...
        "by_repo": {
            repo_id: _compute_metrics(
                operation_rows=[
                    row for row in operation_rows if str(row["repo_id"]) == repo_id
                ],
                read_rows=[row for row in read_rows if str(row["repo_id"]) == repo_id],
                write_rows=[
                    row for row in write_rows if str(row["repo_id"]) == repo_id
                ],
                checkpoint_rows=[
                    row for row in checkpoint_rows if str(row["repo_id"]) == repo_id
                ],
            )
        ...
        "by_host": {
            host_app: _compute_metrics(
                operation_rows=[
                    row for row in operation_rows if str(row["host_app"]) == host_app
                ],
                read_rows=[
                    row for row in read_rows if str(row["host_app"]) == host_app
                ],
                write_rows=[
                    row for row in write_rows if str(row["host_app"]) == host_app
                ],
                checkpoint_rows=[
                    row for row in checkpoint_rows if str(row["host_app"]) == host_app
                ],
            )
...
def _compute_metrics(
    *,
    ...
    for thread_key, reads in thread_reads.items():
        ...
        startup_window_read_count += sum(
            1 for row in reads if _thread_row_time(row) <= startup_window_end
        )
        if len(reads) >= 2:
        ...
        if any(
            any(
                _thread_row_time(op_row) < _thread_row_time(read_row)
                and str(op_row["command"]) != "read"
                for op_row in ops
            )
    ...
    for thread_key, writes in thread_writes.items():
        ...
        for write_row in writes:
            if any(
                event_time < _thread_row_time(write_row) for event_time in event_times
            ):
    ...
    for thread_key, first_guidance_at in guidance_threads.items():
        ...
        if any(
            _thread_row_time(write_row) > first_guidance_at
            and str(write_row.get("update_type")) in _UTILITY_VOTE_UPDATE_TYPES
            for write_row in writes
        ):
    ...
    for thread_key, checkpoints in thread_checkpoints.items():
        ...
        for index, checkpoint in enumerate(checkpoints):
            ...
            checkpoint_deadline = _thread_row_time(checkpoint) + timedelta(
                minutes=_CHECKPOINT_TO_READ_WINDOW_MINUTES
            )
            if (
                next_checkpoint_at is not None
                and next_checkpoint_at < checkpoint_deadline
            ):
    ...
    avg_invocation_latency_ms = _average(
        [int(row["total_latency_ms"]) for row in operation_rows]
    )
    ...
    return {
        ...
        "mid_session_reread_rate": _safe_rate(
            mid_session_reread_threads, len(read_threads)
        ),
        ...
        "read_after_other_action_rate": _safe_rate(
            read_after_other_action_threads, len(read_threads)
        ),
        "read_after_other_action_thread_count": read_after_other_action_threads,
        "startup_window_read_concentration": _safe_rate(
            startup_window_read_count, total_reads
        ),
        "zero_result_read_rate": _safe_rate(
            sum(1 for row in read_rows if bool(row.get("zero_results"))), total_reads
        ),
        "events_before_write_compliance": _safe_rate(
            write_preceded_by_events, total_writes
        ),
        "writes_preceded_by_events": write_preceded_by_events,
        "utility_vote_followthrough": _safe_rate(
            utility_followthrough, len(guidance_threads)
        ),
        "utility_vote_opportunity_count": len(guidance_threads),
        "same_signature_reread_rate": _safe_rate(
            same_signature_read_checkpoint_count, repeated_read_checkpoint_count
        ),
        "same_signature_reread_count": same_signature_read_checkpoint_count,
        "checkpoint_to_read_rate": _safe_rate(
            checkpoint_to_read_count, read_checkpoint_count
        ),
        ...
        "read_to_useful_write_rate": _safe_rate(
            reread_then_write_threads, threads_with_rereads
        ),
...
def _guidance_contains_pending_utility_votes(guidance_codes: object) -> bool:
    ...
    if isinstance(guidance_codes, str):
        ...
        try:
            ...
            return False
        return isinstance(parsed, list) and "pending_utility_votes" in {
