"""Agent operation workflow for applying concept graph changes."""
...
def run_concept_changes_operation(
    ...
    *,
    ...
    """Dispatch a typed concept endpoint payload."""
    ...
    try:
        if validation_errors:
            ...
            result = error_response([ErrorDetail(code=ErrorCode.SCHEMA_ERROR, message="concept request is required")])
        else:
            with uow_factory() as uow:
                result = execute_concept_command(request, uow).model_dump(mode="python")
    except ValueError as exc:
        error_stage = "semantic_validation"
        result = error_response([ErrorDetail(code=ErrorCode.SEMANTIC_ERROR, message=str(exc))])
