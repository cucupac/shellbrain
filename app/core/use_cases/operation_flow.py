"""Coordinate agent operation workflows from validated endpoint payloads."""
...
from app.core.contracts.request_hydration import (
...
from app.core.contracts.agent_requests import (
...
from app.core.policies.telemetry.operation_summary import (
...
from app.core.policies.validation.integrity_validation import validate_create_integrity, validate_update_integrity
from app.core.policies.validation.semantic_validation import validate_create_semantics, validate_update_semantics
...
def run_create_memory_operation(
    ...
    *,
    ...
    session_manager = SessionStateManager(store=dependencies.session_state_store)
    ...
    try:
        ...
        if policy_errors:
            ...
            if errors:
                ...
                resolved_defaults = defaults if defaults is not None else dependencies.create_policy.hydration_defaults()
                ...
                if contract_errors:
                    ...
                    with uow_factory() as uow:
                        validation_errors = _validate_create_request(request, uow=uow, gates=list(policy.gates))
...
def run_concept_changes_operation(
    payload: dict,
    *,
    dependencies: OperationDependencies,
    uow_factory,
    ...
    *,
    ...
    try:
        ...
        if errors:
            ...
            assert agent_request is not None
            resolved_defaults = defaults if defaults is not None else dependencies.read_settings.hydration_defaults()
            ...
            if contract_errors:
                ...
                assert request is not None
                with uow_factory() as uow:
                    result = _execute_read_memory_with_dependencies(request=request, uow=uow, dependencies=dependencies).model_dump(mode="python")
    except Exception as exc:  # pragma: no cover - defensive fallback envelope
        error_stage = "internal_error"
        result = _error_response([ErrorDetail(code=ErrorCode.INTERNAL_ERROR, message=str(exc))])
    ...
    try:
        ...
        if errors:
            ...
            if contract_errors:
                ...
                assert request is not None
                with uow_factory() as uow:
                    result = _execute_recall_memory_with_dependencies(request=request, uow=uow, dependencies=dependencies).model_dump(mode="python")
        ...
        if policy_errors:
            ...
            if errors:
                ...
                if contract_errors:
                    ...
                    with uow_factory() as uow:
                        validation_errors = _validate_update_request(request, uow=uow, gates=list(policy.gates))
                        if validation_errors:
                            error_stage = infer_error_stage_from_errors(
                                _dump_errors(validation_errors),
                                default_stage="semantic_validation",
...
def _execute_read_memory_with_dependencies(*, request, uow, dependencies: OperationDependencies) -> OperationResult:
    try:
...
def _execute_recall_memory_with_dependencies(*, request, uow, dependencies: OperationDependencies) -> OperationResult:
    try:
...
def _selection_summary_from_runtime_context(
    *,
    ...
    if caller_identity is None or caller_identity.trust_level != IdentityTrustLevel.TRUSTED:
        return dependencies.summarize_runtime_selection(repo_root=repo_root, repo_id=repo_id, uow=uow)
