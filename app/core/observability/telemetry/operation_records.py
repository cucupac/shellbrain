...
def build_write_summary_records(
    *,
    ...
    for ordinal, effect in enumerate(effects, start=1):
        ...
        elif effect_type is EffectType.MEMORY_ARCHIVE_STATE and isinstance(params, MemoryArchiveStateEffectParams) and params.archived:
...
def _target_memory_id_from_create(planned_side_effects: list[PlannedEffect]) -> str:
    ...
    for effect in planned_side_effects:
        if effect.effect_type is not EffectType.MEMORY_CREATE:
            continue
        params = effect.params
        if isinstance(params, MemoryCreateEffectParams):
...
def _coerce_planned_effect(effect: PlannedEffect | dict[str, Any]) -> PlannedEffect:
    ...
    resolved_type = EffectType(str(effect_type))
    return PlannedEffect(effect_type=resolved_type, params=_coerce_effect_params(resolved_type, params))


def _coerce_effect_params(effect_type: EffectType, params: dict[str, Any]) -> EffectParams:
    """Normalize serialized effect params back to their typed payload contract."""

    if effect_type is EffectType.MEMORY_CREATE:
        return MemoryCreateEffectParams(**params)
