...
def assemble_context_pack(
    ...
    *,
    ...
    selected_by_bucket: dict[str, list[dict[str, Any]]] = {
        bucket_name: [] for bucket_name in _BUCKET_ORDER
    }
    ...
    sections = {
        "direct": [
            _shape_item(candidate, "direct")
            for candidate in selected_by_bucket["direct"]
        ],
        "explicit_related": [
            _shape_item(candidate, "explicit")
            for candidate in selected_by_bucket["explicit"]
        ],
        "implicit_related": [
            _shape_item(candidate, "implicit")
            for candidate in selected_by_bucket["implicit"]
        ],
    }
