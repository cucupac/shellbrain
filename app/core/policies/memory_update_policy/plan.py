...
def build_update_plan(payload: dict[str, Any], *, plan_ids: UpdatePlanIds) -> list[PlannedEffect]:
    ...
    if update_type == "archive_state":
        return [
            make_side_effect(
                ...
                MemoryArchiveStateEffectParams(memory_id=memory_id, archived=update["archived"]),
            )
        ]
    ...
    if update_type == "utility_vote":
        return [
            make_side_effect(
                ...
                UtilityObservationAppendEffectParams(
                    id=_required(plan_ids.utility_observation_id, "utility_observation_id"),
                    memory_id=memory_id,
    ...
    if update_type == "association_link":
        ...
        return [
            make_side_effect(
                ...
                AssociationUpsertAndObserveEffectParams(
                    repo_id=repo_id,
                    edge_id=_required(plan_ids.association_edge_id, "association_edge_id"),
                    ...
                    observation_id=_required(plan_ids.association_observation_id, "association_observation_id"),
