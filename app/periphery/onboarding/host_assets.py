...
_CURSOR_STATUSLINE_MARKER = "app.periphery.identity.cursor_statusline"
...
def _load_packaged_text(*parts: str) -> str:
    """Return one packaged text asset from app.onboarding_assets."""

    return resources.files("app.onboarding_assets").joinpath(*parts).read_text(encoding="utf-8")
...
def _cursor_statusline_command() -> str:
    ...
    return shlex.join([str(Path(sys.executable).resolve()), "-m", "app.periphery.identity.cursor_statusline"])
