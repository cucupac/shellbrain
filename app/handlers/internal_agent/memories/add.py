...
from app.handlers.internal_agent.memories.utility_vote_evidence import (
    attach_guidance,
            ...
            result = error_response(
                [
                    ErrorDetail(
                        ...
                        message="create request is required",
                    )
                ]
            )
        else:
            with uow_factory() as uow:
                ...
                if result.get("status") == "ok":
                    ...
                    if request.memory.kind == "problem":
                        ...
                        request.memory.kind in {"solution", "failed_tactic"}
                        and request_links.problem_id
                    ):
                        session_state = session_manager.record_problem(
                            repo_root=resolved_repo_root,
                            caller_identity=resolved_telemetry_context.caller_identity,
