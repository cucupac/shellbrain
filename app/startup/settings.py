"""Startup-owned loading for app-packaged YAML settings."""
...
class YamlConfigProvider:
    ...
    def __init__(self, defaults_dir: Path) -> None:
        ...
        self._internal_agents = self._load_yaml_from(
            self._settings_dir / "internal-agents" / "defaults.yaml"
        )
    ...
    def _load_yaml(self, filename: str) -> dict[str, Any]:
        ...
        return self._load_yaml_from(path)

    def _load_yaml_from(self, path: Path) -> dict[str, Any]:
        """This method parses a YAML file from an explicit path."""
    ...
    def get_internal_agents(self) -> dict[str, Any]:
        """This method returns internal-agent configuration values."""

        return deepcopy(self._internal_agents)
