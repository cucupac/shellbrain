...
from app.infrastructure.local_state import machine_config_store, session_state_file_store
...
def install_repo_claude_hook(*, repo_root: Path) -> Path:
    ...
    return claude_hook_install.install_claude_hook(repo_root=repo_root)


def install_managed_host_assets(*, host_mode: str, force: bool):
    """Install Shellbrain-managed host assets."""

    return host_assets.install_host_assets(host_mode=host_mode, force=force)


def load_session_state(*, repo_root: Path, caller_id: str):
    """Load one file-backed session state."""

    return session_state_file_store.FileSessionStateStore().load(repo_root=repo_root, caller_id=caller_id)


def delete_session_state(*, repo_root: Path, caller_id: str) -> None:
    """Delete one file-backed session state."""

    session_state_file_store.FileSessionStateStore().delete(repo_root=repo_root, caller_id=caller_id)
