        ...
        with _acquire_init_lock():
            ...
            if machine_config is None:
                if selection.runtime_mode == RUNTIME_MODE_MANAGED_LOCAL:
                    ...
                    machine_config = _build_external_machine_config(admin_dsn=selection.admin_dsn)
                save_machine_config(machine_config)
                mutated_machine = True
...
def _recover_machine_config_from_docker() -> MachineConfig | None:
    ...
    if shutil.which("docker") is None:
        return None
    try:
        return managed_runtime.recover_machine_config_from_docker(embeddings=_runtime_embeddings_config())
    except FileNotFoundError:
        return None
