...
from app.infrastructure.postgres_admin.connection import fetch_schema_revision, wait_for_postgres
    ...
    *,
    ...
    return run_initialize_runtime(
        ...
        ports=InitializeRuntimePorts(
            ...
            recover_machine_config_from_docker=_recover_machine_config_from_docker,
            ...
            prewarm_embeddings=_prewarm_embeddings,
...
def _build_fresh_machine_config() -> MachineConfig:
    ...
    return managed_runtime.build_fresh_machine_config(embeddings=_runtime_embeddings_config())
...
def _apply_schema_migrations(config: MachineConfig) -> bool:
    ...
    before_revision = fetch_schema_revision(config.database.admin_dsn)
    try:
        upgrade_database()
    except DatabaseRevisionAheadOfInstalledPackageError as exc:
        raise InitConflictError(str(exc)) from exc
    after_revision = fetch_schema_revision(config.database.admin_dsn)
...
def _recover_machine_config_from_docker() -> MachineConfig | None:
    ...
    return recover_managed_machine_config_from_docker(embeddings=_runtime_embeddings_config())
