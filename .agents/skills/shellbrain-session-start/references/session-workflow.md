...
Shellbrain is available as a global CLI. Verify availability:
...
Assume Shellbrain comes from a one-time global install. Only if `shellbrain` is not found should you ask the operator to restore the machine-level install. Do not create per-repo installs unless the operator explicitly wants that.
...
- No active host session found
  Verify that the user is working in Claude Code or Codex, that transcript files exist, and that `repo_root` matches the repo used in that session.
