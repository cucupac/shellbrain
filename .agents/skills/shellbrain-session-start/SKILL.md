...
1. Shellbrain is available as a global CLI. Verify with `shellbrain --help`.
...
3. Start with focused `read` queries about the concrete problem, subsystem, decision, or constraint you are working on. Do not start with vague prompts like "what should I know about this repo?"
4. Use a startup read bundle:
   - prior attempts:
...
5. Inspect the returned context pack:
...
6. Re-run `read` liberally during the task whenever the search shifts, you hit a new subproblem, or you suspect the right memory will only become relevant mid-journey.
7. Before `create` or any evidence-bearing `update`, run `shellbrain events --json '{"limit":10}'`.
8. Reuse returned `data.events[].id` values verbatim as `evidence_refs`.
9. At session end, normalize the episode into durable memories:
   - store the `problem`
...
10. Use the exact payload shapes in [references/request-shapes.md](references/request-shapes.md).
